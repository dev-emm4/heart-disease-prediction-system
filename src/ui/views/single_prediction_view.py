...
from ui.utils.formatters import fmt_timestamp, fmt_feature_vector
...
class SinglePredictionView(tk.Frame):
    ...
    def __init__(self, parent, controller, notify):
        ...
        self._predict_btn: ttk.Button | None = None   # set in _inject_action_bar
        self._build()
    ...
    # ── Feature panel + injected action bar ─────────────────────────────────────
    ...
    def _build_result_panel(self, parent):
        ...
        tk.Label(self._placeholder_frame, text="🫀",
                 font=("Segoe UI", 36),
                 bg=COLORS["surface"], fg=COLORS["border"]).pack(pady=(40, 8))
        tk.Label(self._placeholder_frame, text="No prediction yet",
                 font=FONTS["heading3"],
        ...
        # Result content (hidden until a prediction runs)
    ...
    def _on_clear(self):
        ...
        self.feature_form.scroll_to_top()
    ...
    def _render_result(self, data: dict):
        ...
        if is_positive:
            icon, heading = "⚠", "Heart Disease: POSITIVE"
            sub  = "Indicators suggest disease may be present."
            bg   = COLORS["danger_bg"]
            fg   = COLORS["danger"]
        else:
            icon, heading = "✓", "Heart Disease: NEGATIVE"
            sub  = "No strong indicators of heart disease detected."
            bg   = COLORS["success_bg"]
            fg   = COLORS["success"]

        self._diagnosis_banner.configure(bg=bg)
        for w in (self._diag_icon, self._diag_text, self._diag_sub):
            w.configure(bg=bg)
        self._diag_icon.configure(text=icon, fg=fg)
        self._diag_text.configure(text=heading, fg=fg)
        self._diag_sub.configure(text=sub)
