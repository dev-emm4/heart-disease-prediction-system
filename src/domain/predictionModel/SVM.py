...
class SVM(PredictionModel):
    def __init__(self):
        self._trainedSVM: InferenceSession = rt.InferenceSession(self._getAddressOfModelFile())
    ...
    def makePrediction(self, featureVectors: list[FeatureVector]) -> list[PredictionResult]:
        ...
        resultsInIntegerFormat: list[float] = \
            self._trainedSVM.run([self._labelName], {self._inputName: twoDimensionalArray})[0]
    ...
    def _transformFeatureVectorListTo2dArray(self, featureVectors: list[FeatureVector]) -> list[list[float]]:
        ...
        for featureVector in featureVectors:
            twoDimensionalArray.append(featureVector.getOptimizedFeaturesForSVM())
