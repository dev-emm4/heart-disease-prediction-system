...
class DecisionTree(PredictionModel):
    def __init__(self):
        self._trainedDT: InferenceSession = rt.InferenceSession(self._getAddressOfModelFile())
    ...
    def makePrediction(self, featureVectors: list[FeatureVector]) -> list[PredictionResult]:
        ...
        resultsInIntegerFormat: list[float] = \
            self._trainedDT.run([self._labelName], {self._input_name: twoDimensionalArray})[0]
