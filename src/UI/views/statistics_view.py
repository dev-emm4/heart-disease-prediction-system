...
class StatisticsView(tk.Frame):
    ...
    def _redraw_chart(self):
        ...
        pad_left  = 148   # space for model label
        pad_right = 80    # space for count label
        pad_top   = 20
        bar_h     = 36
        bar_gap   = 22
        bar_area_w = cw - pad_left - pad_right
        ...
        for i, model in enumerate(models):
            ...
            pct = f"{ratio * 100:.1f}%"
            c.create_text(
                x_left + bar_area_w + 10, (y_top + y_bottom) // 2,
                text=f"{count}  ({pct})",
                anchor=tk.W,
                font=FONTS["body"],
